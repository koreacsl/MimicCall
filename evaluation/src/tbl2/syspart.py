...
FILTER_DIR = "./datasets/filters/syspart"
...
def load_syspart_syscalls(out_path):
    try:
        with open(out_path, "r") as f:
            content = f.read()
            match = re.search(r"SYSCALLS\s*\[(.*?)\]", content, re.DOTALL)
            if match:
                raw_syscalls = match.group(1)
                return set(s.strip() for s in raw_syscalls.split(",") if s.strip())
    except Exception as e:
        print(f"[!] {out_path} 파싱 실패: {e}")
    return set()

def evaluate_syspart():
    ...
    for binary in os.listdir(FILTER_DIR):
        ...
        out_path = os.path.join(binary_dir, "serving_syscalls.out")
        if not os.path.isfile(out_path):
            continue

        syscall_set = load_syspart_syscalls(out_path)
        if not syscall_set:
            continue
